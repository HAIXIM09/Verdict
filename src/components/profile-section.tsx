...
const lastBattleAuraChange = 120;
const weekAuraChange = -30;

const allBadges = ['Hot Streak', 'Case Master', 'Top 10', 'Unstoppable', 'Crowd Favorite', 'Perfect Argument'];
const recentBattleResults = [
        ...
        <CardContent className="px-6">
          <div className="flex items-center gap-4">
            ...
            <div className="min-w-0">
              ...
              <p className="text-sm text-stone-400 mt-0.5">Member since {formattedDate}</p>
            </div>
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent className="px-6">
          <div className="flex flex-col sm:flex-row sm:items-center gap-4">
            <div className="flex-shrink-0">
              <p className="text-4xl font-bold text-orange-600">{user.aura.toLocaleString()}</p>
                ...
                return (
                  <div key={i} className="flex-1 flex flex-col items-center gap-1">
                    <div className="w-full flex justify-center">
                      <div
                        className={`w-full max-w-[32px] rounded-sm ${isPositive ? 'bg-orange-600' : 'bg-red-400'}`}
                ...
                <div className="flex items-center gap-3 ml-3 shrink-0">
                  <span
                    className={`text-sm font-semibold ${
                      battle.result === 'won' ? 'text-green-600' : 'text-red-500'
                    }`}
                  >
                  ...
                  <span
                    className={`text-sm font-medium ${
                      battle.auraChange >= 0 ? 'text-[#4D7C0F]' : 'text-red-500'
                    }`}
                  >
                    {battle.auraChange >= 0 ? '+' : ''}{battle.auraChange} Aura
                  </span>
                </div>
              </div>
            ))}
